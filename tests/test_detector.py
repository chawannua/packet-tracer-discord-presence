...
def test_find_packet_tracer_with_stale_cache():
    ...
    detector._cached_pid = 1234
    detector._cached_process = mock_proc
    with patch('psutil.pid_exists', return_value=True), patch('psutil.process_iter', return_value=[]):
...
def test_find_packet_tracer_scan_found():
    detector = ProcessDetector()
    mock_proc = MagicMock()
    mock_proc.info = {'pid': 5678, 'name': 'PacketTracer.exe', 'exe': 'C:\\Program Files\\Cisco Packet Tracer 8.2\\bin\\PacketTracer.exe'}
    with patch('psutil.process_iter', return_value=[mock_proc]):
        assert detector.find_packet_tracer() is True
        assert detector._cached_pid == 5678
        assert detector._cached_process == mock_proc
        assert detector._running_exe_path == 'C:\\Program Files\\Cisco Packet Tracer 8.2\\bin\\PacketTracer.exe'

def test_find_packet_tracer_scan_not_found():
    detector = ProcessDetector()
    mock_proc = MagicMock()
    mock_proc.info = {'pid': 9999, 'name': 'chrome.exe', 'exe': 'C:\\Program Files\\Google\\Chrome\\chrome.exe'}
    with patch('psutil.process_iter', return_value=[mock_proc]):
        assert detector.find_packet_tracer() is False
        assert detector._cached_pid is None
...
def test_find_packet_tracer_scan_handles_exceptions():
    ...
    def mock_iter_gen(*args, **kwargs):
        ...
        mock_alive.info = {'pid': 4321, 'name': 'PacketTracer8.exe', 'exe': 'C:\\PacketTracer8.exe'}
        yield mock_alive
...
def test_find_packet_tracer_prefers_oldest_of_multiple_matches():
    detector = ProcessDetector()
    newer = MagicMock()
    newer.info = {'pid': 111, 'name': 'PacketTracer.exe', 'exe': 'C:\\PT\\PacketTracer.exe', 'create_time': 200.0}
    newer.status.return_value = 'running'
    older = MagicMock()
    older.info = {'pid': 222, 'name': 'PacketTracer.exe', 'exe': 'C:\\PT\\PacketTracer.exe', 'create_time': 100.0}
...
def test_get_installation_paths():
    detector = ProcessDetector()
    detector._running_exe_path = 'C:\\Program Files\\Cisco Packet Tracer\\bin\\PacketTracer.exe'
    with patch('os.environ.get', return_value='C:\\fake'), patch('glob.glob', return_value=[]), patch('os.path.isdir', return_value=False):
        paths = detector.get_installation_paths()
        assert 'C:\\Program Files\\Cisco Packet Tracer\\bin' in paths