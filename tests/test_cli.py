...
def test_cli_main_loop():
    ...
    with patch('sys.argv', test_args), patch('packet_tracer_presence.cli.ProcessDetector') as mock_detector_class, patch('packet_tracer_presence.cli.WindowParser') as mock_parser_class, patch('packet_tracer_presence.cli.RPCManager') as mock_rpc_class, patch('time.sleep', side_effect=KeyboardInterrupt) as mock_sleep:
...
def test_cli_main_loop_closed():
    test_args = ['packet_tracer_presence']
    with patch('sys.argv', test_args), patch('packet_tracer_presence.cli.ProcessDetector') as mock_detector_class, patch('packet_tracer_presence.cli.WindowParser') as mock_parser_class, patch('packet_tracer_presence.cli.RPCManager') as mock_rpc_class, patch('time.sleep', side_effect=[None, KeyboardInterrupt]) as mock_sleep:
        mock_detector = mock_detector_class.return_value
...
def test_cli_main_loop_default_exits_on_close():
    test_args = ['packet_tracer_presence']
    with patch('sys.argv', test_args), patch('packet_tracer_presence.cli.ProcessDetector') as mock_detector_class, patch('packet_tracer_presence.cli.WindowParser') as mock_parser_class, patch('packet_tracer_presence.cli.RPCManager') as mock_rpc_class, patch('time.sleep') as mock_sleep:
        mock_detector = mock_detector_class.return_value
        mock_detector.find_packet_tracer.side_effect = [True, False]
        mock_parser = mock_parser_class.return_value
        state = PacketTracerState(file_name='Workspace', is_unsaved=False)
...
def test_ensure_single_instance_writes_lock_file(tmp_path):
    ...
    with patch('packet_tracer_presence.cli._running_under_tests', return_value=False), patch('packet_tracer_presence.cli.LOCK_FILE', str(lock)):
...
def test_ensure_single_instance_refuses_when_live_instance_running(tmp_path):
    ...
    with patch('packet_tracer_presence.cli._running_under_tests', return_value=False), patch('packet_tracer_presence.cli.LOCK_FILE', str(lock)), patch('psutil.pid_exists', return_value=True), patch('psutil.Process', return_value=live):
...
def test_ensure_single_instance_reclaims_stale_lock(tmp_path):
    lock = tmp_path / '.daemon.lock'
    lock.write_text('4242')
    with patch('packet_tracer_presence.cli._running_under_tests', return_value=False), patch('packet_tracer_presence.cli.LOCK_FILE', str(lock)), patch('psutil.pid_exists', return_value=False):
        assert cli.ensure_single_instance() is True
    assert lock.read_text().strip() == str(os.getpid())
...
def test_main_mutes_comtypes_logger():
    ...
    test_args = ['packet_tracer_presence']
    with patch('sys.argv', test_args), patch('packet_tracer_presence.cli.ProcessDetector') as mock_detector_class, patch('packet_tracer_presence.cli.WindowParser'), patch('packet_tracer_presence.cli.RPCManager'), patch('time.sleep', side_effect=KeyboardInterrupt):
...
def test_main_uses_rotating_log_handler():
    test_args = ['packet_tracer_presence']
    with patch('sys.argv', test_args), patch('packet_tracer_presence.cli.RotatingFileHandler') as mock_handler, patch('packet_tracer_presence.cli.ProcessDetector') as mock_detector_class, patch('packet_tracer_presence.cli.WindowParser'), patch('packet_tracer_presence.cli.RPCManager'), patch('time.sleep', side_effect=KeyboardInterrupt):
        mock_detector_class.return_value.find_packet_tracer.return_value = False
        main()
...
def test_idle_poll_uses_slower_interval_while_packet_tracer_closed():
    test_args = ['packet_tracer_presence', '--no-exit-on-close']
    with patch('sys.argv', test_args), patch('packet_tracer_presence.cli.ProcessDetector') as mock_detector_class, patch('packet_tracer_presence.cli.WindowParser'), patch('packet_tracer_presence.cli.RPCManager'), patch('time.sleep', side_effect=KeyboardInterrupt) as mock_sleep:
        mock_detector_class.return_value.find_packet_tracer.return_value = False
        main()
...
def test_remove_lock_file_is_a_noop_under_tests(tmp_path):
    lock = tmp_path / '.daemon.lock'
    lock.write_text('4242')
    with patch('packet_tracer_presence.cli.LOCK_FILE', str(lock)), patch('packet_tracer_presence.cli._running_under_tests', return_value=True):
...
def test_remove_lock_file_leaves_another_instances_lock_alone(tmp_path):
    ...
    with patch('packet_tracer_presence.cli.LOCK_FILE', str(lock)), patch('packet_tracer_presence.cli._running_under_tests', return_value=False):
        cli.remove_lock_file()
    assert lock.exists()
