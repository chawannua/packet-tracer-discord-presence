        ...
        if auto is not None and not getattr(WindowParser, '_uia_disabled', False):
            ...
            for win in pt_wins:
                try:
                    ...
                    # Device Window:
                    if state.active_device and name.startswith(state.active_device) and cname in (
                            "CWorkstationDialog", "CRouterDialog", "CSwitchDialog", "CDeviceDialog"):
                        sub_title = ""
                        selected_tab = None
                        terminal_text = ""
                        try:
                                ...
                                try:
                                        ...
                                        if sel_item and sel_item.IsSelected and c_name:
                                            ...
                                            if sel_pat:
                                                try:
                                                    ...
                                                    pass
                                            if not selected_tab and c_name and c_name in (
                                                    "Physical", "Config", "CLI", "Desktop",
                                                    "Services", "Programming", "Attributes"):
                                                selected_tab = c_name
                                    ...
                                    if c_class == "QLabel" and "m_titleLabel" in c_auto_id and c_name:
                                        ...
                                        for app in KNOWN_APPLETS:
                                            if app in c_name and (c_class in (
                                                    "QMdiSubWindow", "QDialog", "QWidget") or
                                                    "Window" in getattr(c, "ControlTypeName", "")):
                                                sub_title = app
                                                break

                                    # Terminal / Command line buffer
                                    if c_class == "CCommandLine" or "CommandLine" in c_class or c_class in (
