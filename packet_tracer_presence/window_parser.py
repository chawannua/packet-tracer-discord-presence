...
TAB_MAPPINGS = {'Physical': 'Physical (Hardware)', 'Config': 'Config Tab', 'CLI': 'CLI', 'Services': 'Services', 'Programming': 'Programming', 'Attributes': 'Attributes'}
KNOWN_APPLETS = ['Terminal', 'Command Prompt', 'IP Configuration', 'Web Browser', 'PC Wireless', 'Text Editor', 'Email', 'Traffic Generator', 'MIB Browser', 'Cisco Webex', 'VPN', 'Dial-up', 'Bluetooth', 'Firewall', 'Netflow Collector', 'IoT IDE']
WORKSPACE_TOOLS = {'Inspect (I)': 'Inspecting Network Components', 'Delete (Del)': 'Deleting Components', 'Place Note (N)': 'Annotating Topology (Notes)', 'Add Simple PDU (P)': 'Testing Connectivity (Simple PDU Ping)', 'Add Complex PDU (C)': 'Sending Complex PDU Packets'}
...
def parse_cli_mode(text: str) -> Optional[str]:
    ...
    if 'Press RETURN to get started' in last:
        return '[Console Connected]'
    if re.search('\\(config-subif\\)#\\s*$', last):
        return '[Sub-Interface Config]'
    if re.search('\\(config-if\\)#\\s*$', last):
        return '[Interface Config]'
    if re.search('\\(config-router\\)#\\s*$', last):
        return '[Routing Config]'
    if re.search('\\(config-line\\)#\\s*$', last):
        return '[Line/Console Config]'
    if re.search('\\(config-vlan\\)#\\s*$', last):
        return '[VLAN Config]'
    if re.search('\\(dhcp-config\\)#\\s*$', last):
        return '[DHCP Config]'
    if re.search('\\(config\\)#\\s*$', last):
        return '[Global Config]'
    if re.search('^[A-Za-z]:\\\\.*>\\s*$', last):
...
class WindowParser:
    ...
    def __init__(self):
        self.base_title_re = re.compile('^Cisco Packet Tracer\\s*-\\s*(.*?)$')
        self.timer_re = re.compile('PT Activity:\\s*(\\d{1,2}:\\d{2}:\\d{2})')

    def _apply_desktop_context(self):
        if os.name == 'nt':
            try:
                user32 = ctypes.windll.user32
                h_desktop = user32.OpenDesktopW('default', 0, False, 511)
                if h_desktop:
        ...
        if auto is not None and (not getattr(WindowParser, '_uia_disabled', False)):
            ...
            try:
                ...
                for child in root.GetChildren():
                    try:
                        if getattr(child, 'ProcessId', None) in pt_pids and getattr(child, 'ControlTypeName', '') == 'WindowControl':
                            pt_wins.append(child)
                        ...
                        try:
                            for c, _ in auto.WalkControl(win):
                                try:
                                    ...
                                    if c_class == 'QTabBar' or 'TabItem' in getattr(c, 'ControlTypeName', ''):
