...
from pypresence.exceptions import ArgumentError, DiscordNotFound, InvalidArgument, InvalidID, PipeClosed
from .config import DISCORD_CLIENT_ID, RECONNECT_TIMEOUT, PRESENCE_BUTTONS, DEVICE_ASSET_KEYS, DEFAULT_SMALL_ASSET
...
class RPCManager:
    ...
    def update(self, project_name: str, is_unsaved: bool, start_time: int, file_type: str='unknown', active_device: str=None, active_sub_app: str=None, device_type: str='pt_logo', activity_timer: str=None, completion_percent: str=None, sim_mode: str='Realtime', view_mode: str='Logical', workspace_tool: str=None):
        if not self.connect():
            return
        state_dict = {'project_name': project_name, 'is_unsaved': is_unsaved, 'file_type': file_type, 'active_device': active_device, 'active_sub_app': active_sub_app, 'device_type': device_type, 'activity_timer': activity_timer, 'completion_percent': completion_percent, 'sim_mode': sim_mode, 'view_mode': view_mode, 'workspace_tool': workspace_tool}
        if state_dict == self._last_state:
            return
        now = time.time()
        if now - self._last_update_time < self.rate_limit:
            return
        if is_unsaved or not project_name or project_name == 'Workspace' or ('Untitled' in str(project_name)) or ('New' in str(project_name)):
        ...
        small_image = DEVICE_ASSET_KEYS.get((device_type or '').lower(), DEFAULT_SMALL_ASSET)
        if active_device:
        ...
        payload = dict(details=details, state=state_str, start=start_time, large_image=large_image, large_text=large_text, small_image=small_image, small_text=small_text)
        try:
            try:
                _call_with_timeout(self.presence.update, PIPE_TIMEOUT, buttons=self._buttons, **payload)
            except (TypeError, InvalidArgument, ArgumentError) as button_err:
                if self._buttons is not None:
                    logger.warning('Discord rejected presence buttons (%s); retrying without them', button_err)
                    self._buttons = None
